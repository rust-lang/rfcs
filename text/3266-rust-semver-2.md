...
The `~` operator is very complicated, it's unclear why `~1.2.3` can update PATCH but `~1.2` can't update MINOR but only PATCH, this mean `~1.2` could just be written `~1.2.0`.
Then it's make `~1` that can upgrade MINOR and PATCH a stick equivalent to `^1`.
It is worth mentioning that `~` behavior is not documented for pre-release, so it's unclear what `~` actually do for pre-release.
The `~` operator should not have an exception for `MAJOR.MINOR` and be more useful.
