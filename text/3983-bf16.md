...
- Native `bf16` arithmetic operations that round directly to `bf16`, rather than producing the same result as the specified through the above sequence, are **not** legal lowerings for scalar `bf16` arithmetic. They produce different result bits for some inputs. Such operations remain available through explicit target-specific intrinsics, where the user has opted in to native semantics.
