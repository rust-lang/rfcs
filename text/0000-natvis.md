...
files for a given crate and store them in the crate metadata. This will be gated
by the existing `-Z unstable-options` flag to ensure it is only used in unstable
Rust. When running the linker, using the `MSVC` toolchain, a `/NATVIS` linker
option would be set for each `.natvis` file. This includes `.natvis` files from
all crate dependencies, if any exist, as well as the current crate and embed them
into the PDB.
