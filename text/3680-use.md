...
Rather than having the method `Use::do_use`, we could have the compiler
directly call `Clone::clone` on a `Use` type. Since `Use` has `Clone` as a
supertrait, any type implementing `Use` will implement `Clone`. This would
avoid needing to have an un-overridable method in the `Use` trait. Users who
want to avoid the elision behavior of `.use` could then call `.clone()` rather
than `.do_use()`.
